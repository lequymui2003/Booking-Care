...
export const ListDoctorClinic = () => {
  const navigate = useNavigate();
  const [options, setOptions] = useState<string[]>([]);
  const { id } = useParams(); // id của phòng khám
  ...
  const [clinicDoctors, setClinicDoctors] = useState<ItemDoctor[]>([]); // Đổi tên từ specialtyDoctors -> clinicDoctors
  const [specialties, setSpecialties] = useState([]);
  const [clinics, setClinics] = useState([]);
  const [doctorTimeSlots, setDoctorTimeSlot] = useState([]);
  const [timeSlots, setTimeSlot] = useState([]);
  const [appointments, setAppointments] = useState([]);
  const [currentClinic, setCurrentClinic] = useState<ItemClinic | null>(null);
  const [specialtyDetails, setSpecialtyDetails] = useState<{
    [key: string]: ItemSpecialty;
  }>({});
  ...
  // Lấy thông tin phòng khám hiện tại từ id trong URL
  useEffect(() => {
    if (clinics && clinics.length > 0 && id) {
      const clinic = clinics?.find(
        (clinic: ItemClinic) => clinic.id === Number(id)
  ...
  useEffect(() => {
    if (doctors && doctors.length > 0 && currentClinic) {
      const filteredDoctors = doctors.filter(
        (doc: ItemDoctor) => doc.clinicId === currentClinic.id
      );
      setClinicDoctors(filteredDoctors);
    }
  }, [doctors, currentClinic]);

  // Tạo map lưu thông tin chuyên khoa theo doctor Id
  useEffect(() => {
    if (clinicDoctors.length > 0 && specialties.length > 0) {
      const specialtyMap: { [key: string]: ItemSpecialty } = {};

      clinicDoctors.forEach((doctor: ItemDoctor) => {
        const specialty = specialties.find(
          (s: ItemSpecialty) => s.id === doctor.specialtiesId
        );
        if (specialty) {
          specialtyMap[doctor.id] = specialty;
        }
      });

      setSpecialtyDetails(specialtyMap);

      // Nếu chưa chọn bác sĩ, chọn bác sĩ đầu tiên mặc định
      if (clinicDoctors.length > 0 && !selectedDoctorId) {
        setSelectedDoctorId(clinicDoctors[0].id.toString());
      }
    }
  }, [clinicDoctors, specialties, selectedDoctorId]);
  ...
  const handleTimeSlotClick = (timeSlot: {
    ...
    const doctorSpecialty = specialtyDetails[selectedDoctorId];
    ...
    const bookingData = {
      ...
      specialtyName: doctorSpecialty?.specialtiesName || "",
        ...
        <div className="tw-flex tw-flex-col tw-gap-10 tw-px-4 tw-my-2">
          <div className="tw-flex tw-gap-1 tw-text-sm">
            <div className="tw-flex tw-gap-1 tw-text-sm tw-text-sky-600">
              ...
              <div>{currentClinic?.clinicName}</div>
            </div>
          </div>
          ...
          <div className="tw-w-full tw-mb-8">
            <h2 className="tw-text-xl tw-font-semibold tw-mb-4">
              Danh sách bác sĩ tại phòng khám {currentClinic?.clinicName}
            </h2>
            ...
            <div className="tw-max-h-[260px] tw-overflow-y-auto tw-pr-2">
              {isLoading ? (
                ...
                <div className="tw-grid tw-grid-cols-1 sm:tw-grid-cols-2 md:tw-grid-cols-3 tw-gap-4">
                  {clinicDoctors.map((doctor: ItemDoctor) => (
                    <div
                      key={doctor.id}
                      ...
                      <div className="tw-flex tw-items-center tw-gap-4">
                        ...
                        <div>
                          ...
                          <p className="tw-text-sm tw-text-gray-600">
                            {specialtyDetails[doctor.id]?.specialtiesName ||
          ...
          {selectedDoctorId && clinicDoctors.length > 0 && (
              ...
              <div className="md:tw-flex tw-flex-col tw-w-full">
                ...
                <div className="tw-flex tw-flex-col tw-gap-1 tw-py-3 md:tw-px-4 md:tw-py-3">
                  {(() => {
                    ...
                    const doctorSpecialty = selectedDoctor
                      ? specialtyDetails[selectedDoctor.id]
                      : null;
                      ...
                      <>
                        <div className="tw-flex tw-flex-col tw-gap-1">
                          ...
                          <div className="tw-text-[13px] tw-text-sky-600 tw-cursor-pointer">
                            <p>{currentClinic.clinicName}</p>
                          </div>
                          <div className="tw-text-[13px] ">
                            <p>{currentClinic.clinicAddress}</p>
                          </div>
                        </div>
                        ...
                        {doctorSpecialty && (
                          <div className="tw-flex tw-gap-1">
                            ...
                            <div className="tw-text-sm tw-content-center">
                              <p>{doctorSpecialty.specialtiesName}</p>
                            </div>
                          </div>
                        )}
                      </>
